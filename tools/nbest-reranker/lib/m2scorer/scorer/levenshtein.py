...
def batch_multi_pre_rec_f1(candidates, sources, gold_edits, max_unchanged_words=2, beta=0.5, ignore_whitespace_casing= False, verbose=False, very_verbose=False, stats=False):
    ...
    if stats == False:
        ...
        row_end_idx = len(lmatrix1)-1
        col_end_idx = len(lmatrix1[0])-1
        stats = {'num_correct':int(stat_correct), 'num_proposed':int(stat_proposed), 'num_gold':int(stat_gold), 'num_src_tokens':len(source_tok), 'edit_distance': lmatrix1[row_end_idx][col_end_idx]}
        return (p, r, f1, stats)
...
def merge_edits_del(e1, e2, target, joiner = str(' ')):
...
def merge_edits(e1, e2, joiner = str(' ')):
