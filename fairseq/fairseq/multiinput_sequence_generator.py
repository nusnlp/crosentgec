...
class MultiInputSequenceGenerator(object):
    def __init__(
        ...
        """Generates translations of a given source sentence and context (sentence/document).
        Args:
    ...
    def generate(self, src_tokens, src_lengths, ctx_tokens, ctx_lengths, beam_size=None, maxlen=None, prefix_tokens=None):
        """Generate a batch of translations."""
        with torch.no_grad():
            return self._generate(src_tokens, src_lengths, ctx_tokens, ctx_lengths, beam_size, maxlen, prefix_tokens)

    def _generate(self, src_tokens, src_lengths, ctx_tokens, ctx_lengths,beam_size=None, maxlen=None, prefix_tokens=None):
        bsz, srclen = src_tokens.size()
        _, ctxlen = ctx_tokens.size()
        ...
        for model in self.models:
            ...
            if hasattr(model, 'auxencoder'):
                auxencoder_out = model.auxencoder(
                    ctx_tokens.repeat(1, beam_size).view(-1, ctxlen),
                    ctx_lengths.expand(beam_size, ctx_lengths.numel()).t().contiguous().view(-1),
                )
                auxencoder_outs.append(auxencoder_out)
            else:
                auxencoder_outs.append(None)
        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                for i, model in enumerate(self.models):
                    ...
                    if hasattr(model, 'auxencoder'):
                        auxencoder_outs[i] = model.decoder.reorder_auxencoder_out(auxencoder_outs[i], reorder_state)

            probs, avg_attn_scores = self._decode(
                tokens[:, :step + 1], encoder_outs, auxencoder_outs, incremental_states)
            if step == 0:
    ...
    def _decode(self, tokens, encoder_outs, auxencoder_outs, incremental_states):
        if len(self.models) == 1:
            return self._decode_one(tokens, self.models[0], encoder_outs[0], auxencoder_outs[0] if auxencoder_outs else None, incremental_states, log_probs=True)

        avg_probs = None
        avg_attn = None
        for model, encoder_out, auxencoder_out in zip(self.models, encoder_outs, auxencoder_outs):
            probs, attn = self._decode_one(tokens, model, encoder_out, auxencoder_out, incremental_states, log_probs=False)
    ...
    def _decode_one(self, tokens, model, encoder_out, auxencoder_out, incremental_states, log_probs):
        with torch.no_grad():
            if incremental_states[model] is not None:
                if auxencoder_out:
                    decoder_out = list(model.decoder(tokens, auxencoder_out, encoder_out,  incremental_states[model]))
                else:
