...
# for edit weighted criterion

def prepare_align_batch(aligns, max_len=None, pad_idx=0):
    """Convert a list of lists into a padded 2d tensor."""
    values = [torch.FloatTensor(align) for align in aligns] # converts ot list of floattensors
    if max_len is None:
...
def get_weighted_alignments(align_file, edit_weight, task):
    ...
    with open(align_file) as falign:
        for sample,alignline  in zip(task.dataset('train'),falign):
            # alignline: 0-0 1-1 2-2 ... (can have missing indices on either side)
            # prepare dictionary of the form t:s where t is the target index in alignment and s is the aligned source word index
            align_dict = dict([(  int(a.split('-')[1]), int(a.split('-')[0]))  for a in alignline.strip().split()])
            alignment = []
            for i, tidx in enumerate(sample['target'][:-1]):
                # if there is an alignment of the target word in the source line and the word matches
                if (i in align_dict):
                    if tgt_dict.symbols[tidx] ==  src_dict.symbols[ sample['source'][align_dict[i]]]:
                        alignment.append(1)
                    else:
                        alignment.append(edit_weight)
                else:
                    alignment.append(edit_weight)
            alignment.append(1)

            alignments.append(alignment)
            assert len(alignments[sample['id']]) == sample['target'].size()[0], 'does not match at index {}'.format(sample['id'])
    return alignments
